...
if (month == 4) {
  ...
  var_names =c("number", "salinity", "temperature", "riv_flow", "Nit_val", "Z_anom", "MaxT_anom", "MinT_anom", "TotalMonthlyRF",       
  ...
  var_names =c( "number", "salinity", "temperature", "riv_flow", "Nit_val", "Z_anom", "MaxT_anom", "MinT_anom", "TotalMonthlyRF",       
               ...
               "prev_autumn_MinT_anom","spring_RF",           "winter_RF",            "prev_autumn_RF",       "spring_dis_ALL",       
               "winter_dis_ALL",       "prev_autumn_dis_ALL",  "atspawn_salinity",     "atspawn_waterT",      "first_spawn_salinity" ,
...
df_train_sm <- as.matrix(df_train_sm %>% dplyr::select(-one_of(string)))
...
remove_vars <- c("atspawn_nitro", "avg_last3_nitro", "avg_last2_nitro", "atspawn_salinity", "atspawn_waterT")
TB_class <- TB_class %>% ungroup() %>% select(-one_of(remove_vars))
...
TB_clsslct <- TB_class[sample(nrow(TB_class),0.86*(nrow(TB_class))),] #selected classification
...
label_name="number"
train_features <- TB_clsslct %>% select(-c(number))
...
xgb_tune1 <- train(x=as.matrix(df_train %>% select(-number)), 
...
xgb_tune2 <-train(x=as.matrix(df_train %>% select(-number)), 
                  y=as.factor(df_train$number), #this set up is different for regression
                  method = 'xgbTree',
                  objective = 'binary:logistic',
                  metric = "error",
...
xgb_tune3 <-train(x=as.matrix(df_train %>% select(-number)), 
                  y=as.factor(df_train$number), #this set up is different for regression
                  method = 'xgbTree',
                  objective = 'binary:logistic',
                  metric = "error",
...
importance_matrix <- xgb.importance(names(df_train %>%  select(-number)), model=model_tuned)
...
xgb_pca <-train(x=as.matrix(df_train %>% select(-number)), 
                y=as.factor(df_train$number), #this set up is different for regression
                method = 'xgbTree',
                preProcess='pca',
                objective = 'binary:logistic',
...
confusionMatrix(predict(xgb_pca, df_train %>% select(-number)), df_train$number)
...
#Process and create train and test ####
TB_rgrs <- TB
#TB_rgrs <- TB_rgrs %>% select(-c( atspawn_nitro ,avg_last2_nitro,avg_last3_nitro ))
...
label_name="number"
train_features <- TB_rgrsslct %>% select(-c(number))
train_features <- mutate_all(train_features, function(x) as.numeric(as.character(x)))
...
#View feature importance/influence from the learnt model
importance_matrix <- t(data.frame(xgb.importance(names(df_train %>%  select(-number)), model=model_tuned)))
print(importance_matrix)
...
#View feature importance/influence from the learnt model
importance_matrix_cp <- xgb.importance(names(df_train %>%  select(-number)), model=model_tuned_cp)
...
manualrfe = function(TB, month){
  ...
  results <- matrix(data=NA, nrow=45, ncol=4)
  ...
  #TB_rgrs <- TB_rgrs %>% select(number, features)
...
#best <- results[results$V3 == max(results$V3),]

}


test <- manualrfe(TB, 4)
# 20 variable model best
...
selec_enviro <- cleaned_TB %>% select("year", "month", "salinity", "temperature", "Nit_val", "MeanMonthlyRF", "river_flow") %>% 
...
#Z_anomaly, maxT, minT
cd_ENV <- cleaned_TB %>% select("year", "month", "Z_anom", "MaxT_anom", "MinT_anom") %>%
...
#response
num <- cleaned_TB %>% select("year", "month", "number") %>% group_by(year, month) %>% summarise(N=length(number), sum=sum(number), mean=mean(number), num_per_haul = sum/N, sd= sd(number, na.rm=TRUE), se=sd/sqrt(N))
